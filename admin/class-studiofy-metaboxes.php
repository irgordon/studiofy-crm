...
class Studiofy_Metaboxes {
	...
	/**
	 * Initialize Hooks
	 * This is the method that was missing and causing your Fatal Error.
	 */
	public function init(): void {
		...
		// Register Custom Columns for all Studiofy CPTs
		$cpts = array( 'studiofy_project', 'studiofy_lead', 'studiofy_invoice', 'studiofy_contract', 'studiofy_session' );
		foreach ( $cpts as $cpt ) {
			add_filter( "manage_{$cpt}_posts_columns", array( $this, 'set_columns' ) );
			add_action( "manage_{$cpt}_posts_custom_column", array( $this, 'render_columns' ), 10, 2 );
		}
	}

	public function enqueue_assets( string $hook ): void {
		global $post;
		// Load Contract Builder JS only on contract pages
	...
	public function render_lead( WP_Post $post ): void {
		...
		echo '<p><label>Email:</label> <input type="email" name="studiofy_email" value="'.esc_attr($email).'" class="widefat"></p>';
		echo '<p><label>Phone:</label> <input type="text" name="studiofy_phone" value="'.esc_attr($phone).'" class="widefat"></p>';
	}
	...
	public function save( int $post_id ): void {
		...
		$keys = [
			...
			'studiofy_square_invoice_id', 'studiofy_email', 'studiofy_phone',
			'studiofy_form_schema', 'studiofy_gallery_ids', 
		...
		foreach ( $keys as $k ) {
			if ( isset( $_POST[ $k ] ) ) {
				if ( in_array( $k, ['studiofy_clauses', 'studiofy_form_schema'] ) ) {
					update_post_meta( $post_id, '_' . $k, wp_kses_post( $_POST[ $k ] ) );
				} else {
					update_post_meta( $post_id, '_' . $k, sanitize_text_field( $_POST[ $k ] ) );
				}
			}
		}
	}
	...
	public function set_columns( array $columns ): array {
		$new = ['cb' => $columns['cb'], 'title' => 'Name'];
		global $post_type;
		
		if ( 'studiofy_project' === $post_type ) {
			$new['client'] = 'Client';
			$new['status'] = 'Status';
		} elseif ( 'studiofy_lead' === $post_type ) {
			$new['email'] = 'Email';
		}
	...
	public function render_columns( string $column, int $post_id ): void {
		switch ( $column ) {
			case 'client':
				$cid = get_post_meta( $post_id, '_studiofy_client_id', true );
				if($cid) {
					global $wpdb;
				...
				break;
			case 'status':
				echo esc_html( get_post_meta( $post_id, '_studiofy_status', true ) );
				break;
			case 'email':
				echo esc_html( get_post_meta( $post_id, '_studiofy_email', true ) );
				break;
		}
	}
}
