...
class InvoiceController {
    ...
    public function render_page(): void {
        ...
        if ($action === 'create' || $action === 'edit') { $this->render_builder(); } 
        elseif ($action === 'items' || $action === 'edit_item') { $this->render_items_page(); } 
    ...
    private function render_builder(): void {
        ...
        if (!$inv) {
            $inv = new \stdClass();
            $inv->id = 0; // Explicitly set ID to 0
    ...
    // ... (handle_save_item, handle_delete_item, handle_print same as v2.2.34, strictly typed) ...
