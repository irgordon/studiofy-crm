...
class InvoiceController {
    ...
    public function init(): void {
        ...
        add_action('admin_post_studiofy_email_invoice', [$this, 'handle_email_invoice']); // New Email Action
    }

    // ... (render_page, render_items_page, render_builder, handle_save, handle_save_item, handle_delete_item, handle_print match v2.2.53) ...
    ...
    private function render_list(): void { 
        ...
        // Find Payment Page URL dynamically
        $payment_page = get_page_by_title('Payment');
    ...
    public function handle_email_invoice(): void {
        check_admin_referer('email_invoice_'.$_GET['id']);
        global $wpdb;
        $id = (int)$_GET['id'];
        
        $invoice = $wpdb->get_row($wpdb->prepare("SELECT i.*, c.email, c.first_name FROM {$wpdb->prefix}studiofy_invoices i LEFT JOIN {$wpdb->prefix}studiofy_customers c ON i.customer_id = c.id WHERE i.id = %d", $id));
        
        if (!$invoice || empty($invoice->email)) {
            wp_die('Customer email not found.');
        }

        $payment_page = get_page_by_title('Payment');
        $pay_url = add_query_arg('invoice_id', $id, $payment_page ? get_permalink($payment_page->ID) : home_url('/payment/'));
        
        $subject = "Invoice #{$invoice->invoice_number} from Studio";
        $message = "Hi {$invoice->first_name},\n\nPlease find your invoice linked below. You can pay securely online.\n\nView Invoice: {$pay_url}\n\nThank you!";
        $headers = ['Content-Type: text/plain; charset=UTF-8'];
        
        wp_mail($invoice->email, $subject, $message, $headers);
        
        // Update status to 'Sent'
        $wpdb->update($wpdb->prefix.'studiofy_invoices', ['status' => 'Sent'], ['id' => $id]);
        
        wp_redirect(admin_url('admin.php?page=studiofy-invoices&msg=sent'));
        exit;
    }
    ...
    public function handle_print(): void { check_admin_referer('print_invoice_'.$_GET['id']); global $wpdb; $id = (int)$_GET['id']; $invoice = $wpdb->get_row($wpdb->prepare("SELECT i.*, c.first_name, c.last_name, c.email, c.phone, c.company, c.address FROM {$wpdb->prefix}studiofy_invoices i LEFT JOIN {$wpdb->prefix}studiofy_customers c ON i.customer_id = c.id WHERE i.id = %d", $id)); if (!$invoice) wp_die('Invoice not found.'); $branding = (array) get_option('studiofy_branding', []); $business_name = !empty($branding['business_name']) ? (string)$branding['business_name'] : 'Photography Studio'; $business_logo_url = !empty($branding['business_logo']) ? (string)$branding['business_logo'] : ''; $admin_email = get_option('admin_email'); $enc = new Encryption(); $customer = new \stdClass(); $customer->name = esc_html($invoice->first_name . ' ' . $invoice->last_name); $customer->company = esc_html($invoice->company); $customer->address = esc_html($enc->decrypt($invoice->address)); $invoice->issue_date_formatted = date('m/d/Y', strtotime($invoice->issue_date)); $invoice->due_date_formatted = date('m/d/Y', strtotime($invoice->due_date)); $invoice->payable_to = "Payable to " . esc_html($business_name) . " Upon Receipt"; $invoice->line_items_data = json_decode($invoice->line_items, true) ?: []; $invoice->subtotal = 0; foreach($invoice->line_items_data as $item) { $invoice->subtotal += ((float)$item['qty'] * (float)$item['rate']); } ob_start(); include STUDIOFY_PATH . 'templates/admin/invoice-template.php'; $html = ob_get_clean(); $html .= '<script>window.onload = function() { window.print(); }</script>'; echo $html; exit; }
}
