...
class ContractController {
    ...
    // ... render_list same as v2.2.28 ...
    private function render_list(): void { global $wpdb; $search = isset($_GET['s']) ? sanitize_text_field($_GET['s']) : ''; $where = $search ? $wpdb->prepare("WHERE c.title LIKE %s", "%$search%") : ''; $sql = "SELECT c.*, cust.first_name, cust.last_name FROM {$wpdb->prefix}studiofy_contracts c LEFT JOIN {$wpdb->prefix}studiofy_customers cust ON c.customer_id = cust.id $where ORDER BY c.created_at DESC"; $rows = $wpdb->get_results($sql); echo '<div class="wrap"><h1 class="wp-heading-inline">Contracts</h1><a href="?page=studiofy-contracts&action=create" class="page-title-action">New Contract</a><hr class="wp-header-end">'; echo '<div class="studiofy-toolbar"><form method="get" action=""><input type="hidden" name="page" value="studiofy-contracts"><label for="search_contracts" class="screen-reader-text">Search</label><input type="search" id="search_contracts" name="s" placeholder="Search contracts..." class="widefat" style="max-width:400px;" value="'.esc_attr($search).'"></form></div>'; if (empty($rows)) { echo '<div class="studiofy-empty-card"><div class="empty-icon dashicons dashicons-edit"></div><h2>No contracts yet</h2><p>Create your first contract with digital signature capture.</p><a href="?page=studiofy-contracts&action=create" class="button button-primary button-large">Create Contract</a></div>'; } else { echo '<table class="wp-list-table widefat fixed striped"><thead><tr><th>Title</th><th>Customer</th><th>Status</th><th>Value</th><th>Actions</th></tr></thead><tbody>'; foreach($rows as $r) { $cust = $r->first_name ? esc_html($r->first_name . ' ' . $r->last_name) : 'Unknown'; $del_url = wp_nonce_url(admin_url('admin-post.php?action=studiofy_delete_contract&id='.$r->id), 'delete_contract_'.$r->id); echo "<tr><td>" . esc_html($r->title) . "</td><td>" . $cust . "</td><td><span class='studiofy-badge " . esc_attr(strtolower($r->status)) . "'>" . esc_html(ucfirst($r->status)) . "</span></td><td>$" . esc_html($r->amount) . "</td><td><a href='?page=studiofy-contracts&action=view&id={$r->id}' class='button button-small'>View</a> <a href='?page=studiofy-contracts&action=edit&id={$r->id}' class='button button-small'>Edit</a> <a href='$del_url' class='button button-small' style='color:#b32d2e;' onclick='return confirm(\"Delete contract?\");'>Delete</a></td></tr>"; } echo '</tbody></table>'; } echo '</div>'; }
    ...
    public function handle_save(): void {
        ...
        if ($linked_post_id === 0) {
            $post_id = wp_insert_post([
                ...
                'post_status' => 'publish', // FIXED: Auto-publish for Elementor
    ...
    private function get_default_contract_template(): string {
        ...
        <div class="studiofy-contract-template">
            ...
            <h3>1. The Parties</h3><p>This Service Contract...</p>
            </div>';
    }
}
