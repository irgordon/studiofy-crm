...
class Menu {
    ...
    private BillingController $billingController; // New Controller
    ...
    public function __construct() {
        ...
        $this->billingController = new BillingController(); // Unified
    ...
    public function init(): void {
        ...
        $this->billingController->init(); // Unified
    ...
    public function render_welcome_page(): void {
        // (Welcome Page content same as previous, untruncated)
        ?>
        <div class="wrap studiofy-welcome-wrap"><div class="studiofy-welcome-panel"><div class="studiofy-logo" style="margin-bottom: 20px;"><svg width="250" height="200" viewBox="0 0 500 400" fill="none" xmlns="http://www.w3.org/2000/svg"><defs><linearGradient id="lens_gradient_std" x1="200" y1="130" x2="300" y2="230" gradientUnits="userSpaceOnUse"><stop stop-color="#4f94d4"/><stop offset="1" stop-color="#2271b1"/></linearGradient></defs><g id="Camera_Icon"><rect x="100" y="80" width="300" height="200" rx="20" fill="black"/><path d="M180 80 L210 40 H290 L320 80 H180 Z" fill="black"/><rect x="120" y="70" width="40" height="10" rx="2" fill="black"/><circle cx="250" cy="180" r="85" fill="white"/><circle cx="250" cy="180" r="75" fill="black"/><circle cx="250" cy="180" r="60" fill="url(#lens_gradient_std)"/><ellipse cx="270" cy="160" rx="20" ry="12" transform="rotate(-45 270 160)" fill="white" fill-opacity="0.4"/><circle cx="230" cy="200" r="5" fill="white" fill-opacity="0.2"/><rect x="115" y="100" width="15" height="160" rx="5" fill="#333333"/></g><g id="Typography"><text x="250" y="340" font-family="Arial, Helvetica, sans-serif" font-size="60" text-anchor="middle" fill="black"><tspan font-weight="900" letter-spacing="2">STUDIOFY</tspan> <tspan font-weight="400" letter-spacing="4"> CRM</tspan></text></g></svg></div><p class="about-text" style="font-size:16px;">Get started by setting up your environment.</p><div class="studiofy-welcome-actions"><a href="<?php echo esc_url(admin_url('admin-post.php?action=studiofy_internal_import&nonce='.wp_create_nonce('internal_import'))); ?>" class="button button-primary button-hero">Yes, Import Demo Data</a><a href="<?php echo esc_url(admin_url('admin.php?page=studiofy-dashboard')); ?>" class="button button-secondary button-hero">No, Skip to Dashboard</a></div></div></div><style>.studiofy-welcome-wrap { display: flex; justify-content: center; align-items: center; min-height: 80vh; background: #f0f0f1; } .studiofy-welcome-panel { background: #fff; padding: 50px; border-radius: 8px; box-shadow: 0 4px 20px rgba(0,0,0,0.05); text-align: center; max-width: 600px; width: 100%; } .studiofy-welcome-actions { margin-top: 40px; display: flex; gap: 20px; justify-content: center; flex-wrap: wrap; } .studiofy-logo svg { max-width: 100%; height: auto; }</style>
        <?php
    }
    ...
    public function enqueue_styles($hook): void {
        ...
        wp_enqueue_media();
        wp_enqueue_editor(); // Required for wp_editor
        ...
        // ... (Gallery scripts same as before) ...
