...
class GalleryController {
    ...
    // ... (routes, get_gallery_files, delete_file, update_file_meta - same as previous) ...
    ...
    public function delete_file(\WP_REST_Request $request): \WP_REST_Response {
        ...
        if ($file) { if(file_exists($file->file_path)) unlink($file->file_path); $wpdb->delete($wpdb->prefix.'studiofy_gallery_files', ['id'=>$id]); return new \WP_REST_Response(['success'=>true],200); }
        return new \WP_REST_Response(['success'=>false],404);
    }
    public function update_file_meta(\WP_REST_Request $request): \WP_REST_Response {
        global $wpdb; $id = $request->get_param('id'); $p = $request->get_json_params();
        $u = $wpdb->update($wpdb->prefix.'studiofy_gallery_files', ['meta_title'=>sanitize_text_field($p['meta_title']),'meta_photographer'=>sanitize_text_field($p['meta_photographer']),'meta_project'=>sanitize_text_field($p['meta_project'])], ['id'=>$id]);
        return new \WP_REST_Response(['success'=>(bool)$u],200);
    }

    /**
     * Handle Chunked Uploads via Streaming
     * FIX: Updated nonce check to match what JS sends (studiofy_upload_chunk)
     */
    public function handle_chunk_upload(): void {
        check_ajax_referer('studiofy_upload_chunk', 'nonce'); // FIXED NONCE NAME
    ...
    // ... (render_page, handle_save, handle_delete_gallery, ajax_create_page - remain unchanged) ...
    public function render_page(): void {
            ...
            <div class="studiofy-explorer-container">
                <div class="studiofy-explorer-sidebar">
                    ...
                    <ul class="studiofy-folder-list">
                        <?php foreach($galleries as $g): $status_icon = $g->page_id ? 'dashicons-admin-page' : 'dashicons-hidden'; ?>
                            <li class="folder-item" data-id="<?php echo $g->id; ?>" data-has-page="<?php echo $g->page_id ? 'true' : 'false'; ?>" tabindex="0" role="button">
                                <span class="dashicons <?php echo $status_icon; ?>"></span> <?php echo esc_html($g->title); ?>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                </div>
                ...
                <div class="studiofy-meta-sidebar" id="meta-sidebar" role="complementary" aria-label="Image Details">
                    ...
                    <div id="meta-content" style="display:none;">
                        ...
                        <div class="meta-actions"><button class="button" id="btn-view-large">View Larger</button><button class="button button-link-delete" id="btn-delete-file">Delete File</button></div>
                    </div>
        ...
        <div id="modal-upload-progress" class="studiofy-modal-overlay studiofy-hidden" role="dialog" aria-modal="true">
            <div class="studiofy-modal" style="width: 400px; text-align: center;">
                ...
                <div class="studiofy-progress-container"><div id="studiofy-progress-bar" class="studiofy-progress-bar"></div></div>
    ...
    public function handle_save(): void { /* ... Save Logic Same as Before ... */ check_admin_referer('save_gallery', 'studiofy_nonce_create'); global $wpdb; $title = sanitize_text_field($_POST['title']); $desc  = sanitize_textarea_field($_POST['description']); $pass  = sanitize_text_field($_POST['password']); $wpdb->insert($wpdb->prefix.'studiofy_galleries', ['title' => $title, 'description' => $desc, 'password' => $pass, 'status' => 'active']); wp_redirect(admin_url('admin.php?page=studiofy-galleries')); exit; }
    public function handle_delete_gallery(): void { /* ... Delete Logic Same as Before ... */ check_admin_referer('del_gal_' . $_GET['id']); global $wpdb; $id = (int)$_GET['id']; $page_id = $wpdb->get_var($wpdb->prepare("SELECT wp_page_id FROM {$wpdb->prefix}studiofy_galleries WHERE id = %d", $id)); if ($page_id) wp_delete_post($page_id, true); $wpdb->delete($wpdb->prefix.'studiofy_galleries', ['id' => $id]); $wpdb->delete($wpdb->prefix.'studiofy_gallery_files', ['gallery_id' => $id]); $upload = wp_upload_dir(); $dir = $upload['basedir'] . '/studiofy_galleries/' . $id; if(is_dir($dir)) { array_map('unlink', glob("$dir/*.*")); rmdir($dir); } wp_redirect(admin_url('admin.php?page=studiofy-galleries')); exit; }
    public function ajax_create_page(): void { /* ... Page Create Logic Same as Before ... */ check_ajax_referer('wp_rest', 'nonce'); if (!current_user_can('manage_options')) wp_send_json_error('Unauthorized'); global $wpdb; $id = (int)$_POST['id']; $gallery = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}studiofy_galleries WHERE id = %d", $id)); if ($gallery->wp_page_id) { $link = get_edit_post_link($gallery->wp_page_id, ''); header('Content-Type: application/json'); wp_send_json_success(['message' => 'Exists', 'redirect_url' => html_entity_decode($link)]); return; } $pass = !empty($gallery->password) ? $gallery->password : wp_generate_password(8, false); $pid = wp_insert_post(['post_title'=>$gallery->title.' - Proofing','post_content'=>'[studiofy_proof_gallery id="'.$id.'"]','post_status'=>'publish','post_type'=>'page','post_password'=>$pass]); if ($pid) { $wpdb->update($wpdb->prefix.'studiofy_galleries', ['wp_page_id'=>$pid, 'password'=>$pass], ['id'=>$id]); header('Content-Type: application/json'); wp_send_json_success(['message'=>'Created','redirect_url'=>html_entity_decode(get_edit_post_link($pid, ''))]); } else { header('Content-Type: application/json'); wp_send_json_error('Failed'); } }
}
