...
class DemoDataManager {
    ...
    public function handle_internal_import(): void {
        ...
        // FIX: Strict check for file existence
        if (!file_exists($internal_file)) {
            wp_die('Demo data file (Studiofy_Demo_data.xml) not found in plugin directory. Please reinstall the plugin.');
        }

        $this->process_xml_file($internal_file);
        delete_transient('studiofy_dashboard_stats');
        
        // Clear redirect so we don't loop back to welcome page
    ...
    private function process_xml_file(string $file_path): void {
        ...
        if(isset($xml->galleries->gallery)) {
            foreach ($xml->galleries->gallery as $g) {
                ...
                $pid = wp_insert_post(['post_title' => (string)$g->title . ' - Demo', 'post_content' => '[studiofy_proof_gallery id="'.$db_id.'"]', 'post_status' => 'publish', 'post_type' => 'studiofy_gal', 'post_password' => (string)$g->password]);
