...
class GalleryWidget extends Widget_Base {
    ...
    public function get_name(): string {
        return 'studiofy_gallery';
    }

    public function get_title(): string {
        return esc_html__('Studiofy Gallery', 'studiofy');
    }
    ...
    protected function register_controls(): void {
        ...
        $this->start_controls_section(
            'section_content',
            [
                'label' => esc_html__('Gallery Settings', 'studiofy'),
                'tab' => Controls_Manager::TAB_CONTENT,
            ]
        );

        // Fetch Folders (Taxonomy Terms)
        ...
        $this->add_control(
            'folder_id',
            [
                'label' => esc_html__('Select Gallery Folder', 'studiofy'),
        ...
        $this->add_control(
            'mode',
            [
                'label' => esc_html__('Display Mode', 'studiofy'),
                'type' => Controls_Manager::SELECT,
                'options' => [
                    'grid' => 'Public Grid',
                    'proofing' => 'Client Proofing (Watermarked)',
                ],
                'default' => 'grid',
            ]
        );
        ...
        $this->start_controls_section(
            'section_style',
            [
                'label' => esc_html__('Style', 'studiofy'),
        ...
        $this->add_control(
            'columns',
            [
                'label' => esc_html__('Columns (Grid)', 'studiofy'),
                'type' => Controls_Manager::NUMBER,
                'min' => 1,
                'max' => 6,
                'default' => 3,
                'selectors' => [
                    '{{WRAPPER}} .studiofy-grid' => 'grid-template-columns: repeat({{VALUE}}, 1fr);',
                ],
            ]
        );
    ...
    protected function render(): void {
        ...
        // Query Logic (Reused from previous Shortcode implementation)
        $args = [
            'post_type' => 'attachment',
            'post_status' => 'inherit',
            'posts_per_page' => -1,
            'tax_query' => [[
                'taxonomy' => 'studiofy_folder',
                'field' => 'term_id',
                'terms' => $folder_id
            ]]
        ];
        ...
        if ($query->have_posts()) {
            ...
            while ($query->have_posts()) {
                ...
                $id = get_the_ID();
                $src = ($mode === 'proofing') 
                    ? $watermarker->apply_watermark($id) 
